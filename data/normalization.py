...
def min_max(data):
  ...
  data_min_max.to_csv('preprocessed/data_min_max.csv')
...
def z_score(data):
  ...
  # Save the normalized data to new CSV files
  data_z_score.to_csv('preprocessed/data_z_score.csv')

def test():
  # Load the cleaned and resampled data from a CSV file
  data = pd.read_csv('preprocessed/cleaned_data.csv', index_col=0, parse_dates=True)
